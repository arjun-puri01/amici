...
- **GoTrue auth lock wedging the app after a long background run.** Symptom: after ~a day of background location, tapping the icon hangs the UI (stuck on loading) while the blue location indicator stays on; force-quit fixes it. Cause: `@supabase/auth-js` serializes all auth ops behind an in-memory lock (RN has no Navigator LockManager). With `autoRefreshToken: true` and NO AppState gating, the refresh timer + the background task's per-minute `supabase.auth.getSession()` contend on that lock 24/7 in the background; if the JS runtime is frozen by iOS while the lock is held (mid-refresh/getSession, no timeout), the lock is never released and the next foreground `getSession()` deadlocks. Hardened (all pure JS, 2026-07): (1) `supabase.ts` gates auto-refresh to foreground via `AppState` (`start/stopAutoRefresh`); (2) `backgroundTask.ts` reads the user id from `sessionCache.ts` (AsyncStorage) instead of `getSession()` every fix; (3) all background network calls (active_windows fetch, ping insert, match-users invoke) wrapped in `withTimeout`. AuthContext keeps the cached id in sync. Requires a multi-hour real-device background test to confirm; simulator won't reproduce.
